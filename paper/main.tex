...
\newcommand{\betaD}{\beta}             % seed mixing (uniform vs seeded)
\newcommand{\rzer}{r_0}                % baseline rank from pvec
...
Invoker ($I$) & Principal & Consumes result; may attest semantics (post-hoc via \texttt{attest\_task\_result}; feeds trust graph, not immediate acceptance) \\
\bottomrule
\end{tabular}
...
\ForAll{$V \in \Vset$}
  \State $(v, c, m) \gets \CP.\textsf{Invoke}(V, \textsf{verify\_result}, \langle \mathrm{taskId}, W, T, d_x, d_y \rangle)$ \Comment{$d_x=H(x)$, $d_y=H(y)$; verifier returns $\{\mathrm{verdict}, \mathrm{confidence}, \mathrm{mode}\}$}
...
\end{equation}
Implementation: $\lambda_{\mathrm{verifier}}=1$, $\lambda_{\mathrm{invoker}}=0.5$; $s(\mathrm{REPLAY})=1$, $s(\mathrm{CROSSCHECK})=0.8$, $s(\mathrm{HEURISTIC})=0.6$; $g$ boosts when multiple attestors concur on the same task (capped 1.2); $\delta=\exp(-\ln 2\cdot\Delta t/\mathit{halfLife})$ with half-life 90 days.
...
\subsection{Seed Vector $\pvec$ and Baseline Rank}
\label{sec:seed}
The seed (teleport) vector $\pvec$ determines the reputation mass each attestor receives when not following the attestation graph. To ensure every attestor---including those with no attestation history---receives a well-defined baseline, we define
\begin{equation}
\pvec = (1-\betaD)\,\mathbf{u} + \betaD\,\mathbf{s},
\end{equation}
where $\mathbf{u}$ is the uniform distribution over $\Aset$ (all attestors who have appeared in any receipt), $\mathbf{s}$ is a seeded distribution (e.g., prior trust or domain-specific weights), and $\betaD \in [0,1]$. The uniform component guarantees $\min_{a \in \Aset} p_a \ge (1-\betaD)/|\Aset|$; we denote this baseline $\rzer$. Choosing $\betaD$ (e.g., $\betaD=0.8$) yields a tunable $\rzer$; default $\rzer \approx 0.2$ for small $|\Aset|$.
...
\end{equation}
Attestors with no attestation history (rows of zeros in $\Cmat$) receive reputation solely from the teleport term; their rank is at least $\rzer$ from $\pvec$.
...
\end{equation}
$\Aset$ is the set of \emph{verifier} attestors that returned \Pass for the task (invoker attestation is asynchronous and does not affect immediate acceptance). Attestors not yet in the attestation graph receive $R_A = \rzer$ from the seed vector $\pvec$ (Section~\ref{sec:seed}). Default $\tauT=0.5$, $\alphaD=0.85$.
\end{definition}
...
The attest-substrate control plane implements ExecutionRank: discovery via ERC-8004 subgraph (search\_agents, list\_tools); policy filters (MIN\_REPUTATION, ALLOW\_RISK\_CLASS); invoke always runs worker + verifiers; receipts stored in SQLite; ranking refreshed periodically (default 60s). Verifier selection: allowlist (capped at 10 agents) or search-based; worker excluded. Verifier tool: \texttt{verify\_result} (fallback \texttt{validate}); input \texttt{\{taskId, workerAgentId, workerTool, workerArgsHash, workerOutputHash\}}; output \texttt{\{verdict, confidence, mode\}}. Digests: SHA-256 of canonical JSON.
...
$\pvec$ & Seed distribution $(1-\betaD)\mathbf{u}+\betaD\mathbf{s}$ \\
$\rzer$ & Baseline rank (from uniform component of $\pvec$) \\
$\alphaD$ & Damping \\
$\betaD$ & Seed mixing (uniform vs seeded) \\
...
\end{lstlisting}
\caption{Execution Receipt (attest-substrate schema). Digests are SHA-256 of canonical JSON.}
\end{figure*}
